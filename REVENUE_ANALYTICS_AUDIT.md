...
| Product cost sources (`_cost`, `_wc_cog_cost`, variation→parent, `faracart_product_cost` filter) | **Already implemented** in `RewardCostEstimator` |
...
`Tracker` → `analytics_events` → `AnalyticsRepository` → `GET /faracart/v1/analytics`
...
(`faracart_attribution_metric_rows` = 5000, `faracart_attribution_order_scan_pages` =
...
- PHP/TS/JS strings all use `__( '…', 'faracart' )`; POT generated by
  `bin/extract-pot.php`, JED/MO by `bin/build-i18n.php`; `fa_IR` translations committed
  (`languages/faracart-fa_IR.po` + JED).
...
faracart_product_cost filter                     ┘        (never invents a cost)
        │
        ▼
...
- **No in-plugin cost-configuration route exists** — `Settings` has no cost/margin
  ...
  `faracart_product_cost` filter. Spec §10's "Set up product costs" CTA therefore needs
