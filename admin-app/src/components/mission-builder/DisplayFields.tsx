...
import type { DisplaySettingsInput, MissionInput } from '../../types';
 ...
 * falls back to the Mission scope default from Settings/Appearance; the
 * settings form is generated generically from the template's schema, so
 * a new template automatically gets a working per-mission settings UI.
 */
export default function DisplayFields({ values, onValueChange }: DisplayFieldsProps) {
  ...
  const definition = templateById(templates, 'mission', templateId);
  const templateSettings = display.template_settings ?? definition?.settings ?? {};

  const chooseTemplate = (next: string) => {
    if (!next) {
      // Reset to the scope default: drop the per-mission override entirely.
      const rest = { ...display };
    ...
    const nextDefinition = templateById(templates, 'mission', next);
  ...
  return (
    <Grid container spacing={2} sx={{ alignItems: 'flex-start' }}>
      ...
      <Grid size={{ xs: 12, sm: 6 }}>
        ...
        >
          <MenuItem value="">
            <em>{__('Default (mission templates)', 'faracart')}</em>
          </MenuItem>
          {(templates?.mission ?? []).map((template) => (
      ...
      <Grid size={12}>
        <TextField
          ...
          helperText={__(
            'Supports {current}, {target}, {remaining}, {percentage}, {quantity}, {reward} and {mission_name}.',
            'faracart'
          )}
      ...
      <Grid size={12}>
        <TextField
          ...
          placeholder={__('Mission reached — your reward is unlocked!', 'faracart')}
