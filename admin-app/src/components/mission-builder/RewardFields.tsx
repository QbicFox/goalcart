...
import type { MissionInput, RewardMetaInput, RewardType, SearchCoupon } from '../../types';
...
export default function RewardFields({ values, onValueChange }: RewardFieldsProps) {
  ...
  const changeType = (next: RewardType) => {
    const patchData: Partial<MissionInput> = {
      reward_type: next,
      reward_value: null,
          ...
          {type === 'free_shipping' && (
            <Grid size={12}>
              <Typography variant="body2" color="text.secondary">
                {__(
                  'Makes every shipping rate free once the mission is reached. Zone/method-specific rules are configured in a later phase.',
                  'faracart'
                )}
              </Typography>
            </Grid>
            ...
            <>
              ...
              {meta.gift_add_mode === 'choose' ? (
                <Grid size={12}>
                  <EntityAutocomplete
                    ...
                    multiple
                    helperText={__(
                      'The customer chooses one free gift from this list once the mission is reached. The first product is the storefront default.',
                      'faracart'
                    )}
                  />
                </Grid>
              ...
              <Grid size={{ xs: 12, sm: 6, lg: 4 }}>
                <TextField
                  ...
                  fullWidth
                  // A mission saved before the 'optional' mode was removed may
...
interface CouponFieldsProps {
  ...
  /** Patches top-level mission reward columns (value/cap). */
  patch: (patch: Partial<MissionInput>) => void;
}
