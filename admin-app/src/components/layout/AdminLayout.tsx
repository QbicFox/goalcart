...
export default function AdminLayout() {
	...
	const anyExpanded = NAV_SECTIONS.some(
		(section) => expanded[section.title] !== false,
	);

	const setAllSections = (value: boolean) => {
		setExpanded(() => {
			const next = Object.fromEntries(
				NAV_SECTIONS.map((section) => [section.title, value]),
			);
            ...
            instead of overlapping it. */}
				...
				<Box
					...
					sx={(theme) => ({
						...
						minWidth: 0,
						...(fullscreen
							? { height: "100%", overflowY: "auto", ...thinScrollbar(theme) }
							: {}),
						p: { xs: 2, md: 3 },
					})}
				>
