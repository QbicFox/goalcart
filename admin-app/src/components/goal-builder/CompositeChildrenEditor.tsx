...
const CHILD_TYPES = GOAL_TYPES.filter((type) => type.value !== 'composite');

function emptyChild(): GoalChildInput {
  return {
    type: 'amount',
    target: 0,
...
/**
 * Goal Builder → Target → Composite children (Phase 9). A composite goal
 * is an AND/OR combination of child goals; this editor maintains the
 * ordered list of child configs (Goal::from_array() payloads) the engine
 * evaluates through the same registry as top-level goals (Phase 4).
 */
export default function CompositeChildrenEditor({
  children,
  onChange,
}: CompositeChildrenEditorProps) {
  const updateChild = (index: number, patch: Partial<GoalChildInput>) => {
  ...
  return (
    <Box>
      ...
      {children.map((child, index) => {
        const childType = child.type as GoalType;
