...
function buildOpportunities(
	...
	if (
		...
		opportunities.push({
			...
			body: sprintf(
				...
				__(
					"Only %s of completed missions were followed by a purchase. Review your mission targets.",
					"faracart",
				),
	...
	if (upsellAssisted > 0) {
		opportunities.push({
			...
			body: sprintf(
				...
				__(
					"Smart Upsells assisted %s mission completions this period.",
					"faracart",
				),
				formatNumber(upsellAssisted),
			),
			action: {
				label: __("View performance", "faracart"),
				to: "/optimization/upsells",
			},
		});
	}
	...
	if (topMission && topMission.attributed_revenue > 0) {
		opportunities.push({
			...
			action: {
				label: __("View missions", "faracart"),
				to: "/revenue/missions",
			},
		});
	}
...
function SalesTrendChart({ data }: { data: RevenueTrendPoint[] }) {
	...
	return (
		<Box
			role="img"
			aria-label={__(
				"Daily attributed sales and mission completions trend",
				"faracart",
			)}
...
function MissionPerformanceCard({
	...
	missions: MissionPerformanceRow[];
	loading: boolean;
}) {
	return (
						...
						{missions.map((mission) => (
							<Box key={mission.mission_id}>
								...
								<LinearProgress
									variant="determinate"
									value={
										mission.completion_rate === null
											? 0
											: Math.min(100, mission.completion_rate * 100)
									}
...
export default function Dashboard() {
	...
	const comparisonQuery = useQuery({
		...
		queryFn: () =>
			fetchRevenueOverview({ from: comparison.from, to: comparison.to }),
	});
	...
	const upsellsQuery = useQuery({
		...
		queryFn: () =>
			fetchUpsellAnalytics({ from: range.from, to: range.to, limit: 50 }),
	});
	...
	const hasData =
		summary !== undefined && (summary.funnel.views > 0 || summary.orders > 0);
	...
	const topMissions = useMemo(() => {
		...
		return [...items]
			.sort(
				(a, b) =>
					b.attributed_revenue - a.attributed_revenue ||
	...
	const upsellTotals = useMemo(
		() => sumUpsells(upsellsQuery.data ?? []),
		[upsellsQuery.data],
	);

	const upsellAssisted = useMemo(
		() =>
			(missionsPerfQuery.data?.items ?? []).reduce(
				(sum, mission) => sum + mission.upsell_assisted,
				0,
			),
		[missionsPerfQuery.data],
	);
	...
	const salesTrend: Trend = {
		change: summary
			? percentChange(
					previous?.mission_driven_revenue,
					summary.mission_driven_revenue,
				)
			: null,
	};
	const completionsTrend: Trend = {
		change: summary
			? percentChange(previous?.funnel.completed, summary.funnel.completed)
			: null,
	};
	...
	return (
		<PageContainer
			...
			description={__(
				"A business summary of your Missions and Smart Upsells.",
				"faracart",
			)}
			actions={
				<Tooltip title={__("Calculate Analytics", "faracart")}>
					...
					>
						{aggregating
							? __("Calculating…", "faracart")
							: __("Calculate Analytics", "faracart")}
					</Button>
				</Tooltip>
			}
		>
			{overviewQuery.isError ? (
				...
				>
					{__(
						"We couldn’t load your analytics. Your Missions are still working normally.",
						"faracart",
					)}
				</Alert>
				...
				<Stack spacing={3}>
					...
					>
						<MissionPerformanceCard
							...
							loading={missionsPerfQuery.isLoading}
						/>
