...
- **UPSELL_REFACTOR — Goal Optimization & Upsell Performance** — the
  ...
  - **Historical cost snapshots** — new `Analytics\OrderCostSnapshot`
    ...
    (`_faracart_unit_cost`, `woocommerce_checkout_create_order_line_item`
    ...
    historical profit. New hook: `faracart_order_cost_snapshot`.
...
- **Phase 2 — Backend/Data Layer (Revenue & Analytics UX simplification)**
  ...
  no new order scans):
  - **`GET /faracart/v1/analytics` summary extended** (§37) with
...
- **Phase 3 — Profit Availability (Revenue & Analytics UX simplification)**
  ...
  - **Cost source chain verified** — `_cost`, `_wc_cog_cost` (read when
    ...
    fallback to the parent, and the `faracart_product_cost` filter.
  - **Variation fallback fix** — a variation inheriting its parent's
    cost now runs the parent through the `faracart_product_cost`
...
- **Phase 33.8 — Testing & Optimization** — the closing testing pass of
  ...
  - **Regression hardening** — the attribution / aggregation /
    ...
    ids). Stale `faracart_rev_*` transients left by interrupted runs are
...
- **Phase 33.7 — Frontend Upsell Integration** — the storefront half of
  ...
  - **Mobile + theme compatibility** — grid on desktop, swipeable
    ...
    the scoped `faracart-*` classes and the existing CSS custom-property
...
- **Phase 33.1 — Revenue Analytics Foundation** — the event layer behind
  ...
  - **Event cleanup** — weekly `faracart_revenue_cleanup` cron
    ...
    `faracart_revenue_retention_days`) in bounded batches and sweeps
    orphan `upsell_stats` rows.
  - **Hooks** — `faracart_revenue_tracking_enabled` and
    `faracart_revenue_retention_days` added to the developer-hooks
    reference.
...
- **Phase 33.2 — Revenue Attribution** — the attribution engine on top of
  ...
  - **Margin & profit impact** — product cost is read from the store's
    ...
    `faracart_product_cost` filter (never modified);
    `estimated_profit = incremental_revenue × margin% − reward_cost −
  ...
  - **Feature flags** — attribution gates on the master + analytics
    toggles plus the new `faracart_attribution_enabled` filter; reads
    are capped by `faracart_attribution_metric_rows` /
...
- **Phase 33.3 — Aggregation & Performance** — the scheduled, cached
  ...
  - **Bounded catch-up** — `aggregate_revenue()` starts the day after
    the last aggregated date (`faracart_revenue_last_aggregated`) or
    the lookback floor (`faracart_aggregate_lookback_days`, aligned
    with retention), processes at most `faracart_aggregate_max_days`
  ...
  - **Cached revenue summaries** — new `RevenueRepository` serves
    ...
    (`faracart_revenue_cache_ttl`) and a master bypass
...
- **Phase 33.4 — Smart Goal Recommendation** — the deterministic
  ...
  - **API + caching** — new admin-only
    ...
    `faracart_recommendation_margin_products`,
    `faracart_recommendations` and
...
- **Phase 33.5 — Smart Upsell Ranking** — the deterministic product
  ...
  - **Historical learning** — the storefront reports upsell
    interactions through the new public `POST /faracart/v1/upsell/track`
  ...
  - **API + caching** — new admin-only
    ...
    `faracart_upsell_candidates` and `faracart_upsells`.
...
- **The `optional` gift add mode is gone.** `gift_add_mode` now accepts
  ...
  stamped `faracart_gift_mode = 'optional'` keep their remove link until
...
- **Persian (fa_IR) translation of the admin dashboard** —
  ...
  drift. Compiled by `bin/build-i18n.php` into `faracart-fa_IR.mo` and
  the `faracart-fa_IR-faracart-admin.json` JED the admin app loads via
...
- **Gutenberg block (P21)** — the progress widget is now available as a
  native Gutenberg block, `faracart/progress` (api version 2, widgets
  ...
  through their shortcode elements (`[faracart_progress ...]`), consistent
...
- **MUI form inputs clashing with WP admin form styles** — WP admin's
  ...
  e.g. the preview dialogs' controls). `:is(#faracart-admin, body
  ...
  `#faracart-admin * { box-sizing: border-box }` out-specifies MUI's
...
- **The storefront progress widgets now update live on every AJAX cart
  ...
  cart-change mechanism into one `faracart:cart-changed` bridge on
...
- **Free-gift rewards: quantity lock, stale removal and selectable mode
  ...
  **Selectable (choose) mode works end-to-end:** choosing a candidate
  ...
  gift line (`faracart_gift_mode`, self-healed on kept legacy lines);
...
- **Choosing a gift in the storefront picker no longer fails with
  "Your cart is empty" (Phase 32/Phase 6).** `POST /faracart/v1/gift`
  rejected every claim with `faracart_gift_empty_cart` because
...
- **Storefront progress widgets were shown to logged-in admins browsing
  ...
  `faracart_admin_capability` capability (default `manage_options` — the
...
- **Storefront analytics went quiet after a cached page or long-lived
  tab — every `/track` report failed with `faracart_invalid_nonce`
  ...
  The public `GET /faracart/v1/progress` payload now mints a fresh
  `faracart_track` nonce (`tracking_nonce`) on every response and
...
- **Storefront progress went stale after cart changes — the widget kept
  ...
  `GET /faracart/v1/progress` response was cacheable; the widget's bare
...
- **All templates looked identical on the storefront — two causes
  ...
  `FARACART_VERSION`, which never changes between releases, so browsers
...
- **P02-T01 Objective** — Created the FaraCart plugin using the exact architectural conventions discovered in Phase 0 (reference: `wooinsights`): namespace `FaraCart\` → `includes/`, singleton bootstrap at file scope, DI container, HookManager, Schema/Installer migration framework, Vite admin-app.
- **P02-T02 Tasks** — Implemented all foundation tasks: plugin slug `faracart` (`faracart.php`); plugin bootstrap; Composer config (PSR-4, PHP ≥ 7.4); frontend build stack (`admin-app/`: Vite 5, TypeScript strict, ESLint 9 flat, Prettier, npm scripts); activation/deactivation/uninstall (`Installer` + `uninstall.php`); plugin constants; WooCommerce dependency checks; WP/PHP/WC minimum version compatibility checks (`Compatibility`, load-order-safe `plugins_loaded` gate); capability checks (`faracart_admin_capability` filter); nonce strategy (`wp_rest` nonce in admin boot data + `X-WP-Nonce` client header); translation loading (`load_plugin_textdomain`); logging strategy consistent with the reference (`error_log` with phpcs annotations).
...
FK-resilient inserts, the `faracart_tracking_enabled` filter, frontend
config print `window.faracartTracking` at `wp_footer` priority 4, and the
...
`POST /faracart/v1/track` endpoint — nonce-guarded (own tracking nonce
...
`enabled` toggle + the `faracart_tracking_enabled` filter (Phase 18 adds
a settings toggle).
...
`GET /faracart/v1/analytics` (`includes/REST/AnalyticsController.php`, DI
...
convention): a best-effort `faracart-debug.log` in `WP_CONTENT_DIR`
...
longer 500s (`faracart_settings_save_failed`), it is treated as a
...
- **P22-T01 PHP** — Full audit of the PHP layer: nonce verification (the
  ...
  `faracart_track`; admin routes rely on WP core `wp_rest` cookie auth),
...
- **P22-T04 Database** — Prepared statements throughout: SQL-injection
  ...
  foreign-key pattern); `FARACART_DB_VERSION` bumped to `0.2.1` so
...
- **P27-T01 Text domain** — Verified and hardened the translation
  posture: the plugin header declares `Text Domain: faracart` and
  `Domain Path: /languages`; `Plugin::load_textdomain()` loads the
  domain on `init`; every PHP string routes through `__()` / `_e()` /
  `esc_html__()` / `_x()` / `sprintf()` with the `faracart` domain; and
...
- **P27-T02 POT generation & build pipeline** — New self-contained
  ...
  `faracart.php`, `includes/` (PHP) and  `admin-app/src` (TS/TSX — the
  same `__( '…', 'faracart' )` syntax) and writes a deterministic
  `languages/faracart.pot` (445 entries — including printf positional
  placeholders like `%1$s` — sorted, deduped, `#: file:line`
  references) with `--check` for CI freshness; `bin/build-i18n.php`
  compiles `languages/faracart-<locale>.po` files into gettext `.mo`
  (verified magic bytes) and the JED JSON the admin loads — named
  `faracart-<locale>-faracart-admin.json` per WP 7's
...
- Added files: `bin/{extract-pot,build-i18n}.php`,
  `languages/faracart.pot`, `tests/i18n-test.php` (44 checks),
