...
check( '/search/coupons registered', route_exists( $routes, '/faracart/v1/search/coupons' ) );
check( '/campaigns registered', route_exists( $routes, '/faracart/v1/campaigns' ) );
check( '/campaigns/{id} registered', route_exists( $routes, '/faracart/v1/campaigns/(?P<id>[\d]+)' ) );
check( '/campaigns/{id}/duplicate registered', route_exists( $routes, '/faracart/v1/campaigns/(?P<id>[\d]+)/duplicate' ) );
check( '/progress registered', route_exists( $routes, '/faracart/v1/progress' ) );
...
$req = new \WP_REST_Request( 'GET', '/faracart/v1/search/categories' );
$req->set_param( 'q', '' );
$req->set_param( 'per_page', 5 );
...
$req = new \WP_REST_Request( 'GET', '/faracart/v1/search/coupons' );
$req->set_param( 'q', '' );
$req->set_param( 'per_page', 5 );
...
$resp = $campaigns_ctrl->handle_index( new \WP_REST_Request( 'GET', '/faracart/v1/campaigns' ) );
$data = $resp->get_data();
...
try {
	...
	$req->set_param( 'frontend_custom_css', '<script>alert(1)</script>.faracart-card { color: red; }' );
	$resp = $settings_ctrl->handle_save( $req );
	...
	$req  = new \WP_REST_Request( 'GET', '/faracart/v1/campaigns/' . $campaign_id );
	$req->set_param( 'id', $campaign_id );
	$resp = $campaigns_ctrl->handle_get( $req );
	check( 'deleted campaign returns 404', is_wp_error( $resp ) && 404 === $resp->get_error_data()['status'] );

	$repo = $container->get( \FaraCart\Campaigns\CampaignRepository::class );
	...
	$admin_id = wp_insert_user( array(
		...
		'user_pass'  => 'test-pass',
		'user_email' => 'faracart-toggle-' . wp_rand( 1000, 9999 ) . '@example.test',
		'role'       => 'administrator',
	) );
	...
	$req = new \WP_REST_Request( 'PUT', '/faracart/v1/campaigns/' . $toggle_campaign_id );
	$req->set_header( 'Content-Type', 'application/json' );
	$req->set_body( wp_json_encode( array( 'status' => 'inactive' ) ) );
	$resp = $server->dispatch( $req );
