...
FaraCart helps WooCommerce store owners **increase Average Order Value (AOV) and conversion** by showing shoppers a clear, motivating path from their current cart to a reachable next mission, and by suggesting the incremental products that close the gap.

The product is **not** a passive progress-bar widget. It is a revenue-optimization engine built on four connected mechanisms:

1. **Missions** — a target the shopper can reach (cart amount, item quantity, category spend, …).
2. **Rewards** — the benefit unlocked when the mission is completed (free shipping, percentage discount, fixed discount, …).
...
Concretely, each feature must answer **yes** to: does it reduce friction, make the next purchase step obvious, increase mission completion, increase cart value, improve conversion, preserve performance, and avoid harming checkout UX?
...
| What makes it different from a plain progress bar? | Missions + rewards + suggestions + campaigns + analytics work together as one engine; the bar is only the visible surface. |
...
A **target the customer can reach** within a session of shopping. Missions are the unit of motivation.

| Mission dimension | Values (MVP) | Notes |
|---|---|---|
...
| **Status** | `active`, `inactive`, `scheduled` | Scheduled missions belong to campaigns (see §2.3). |
| **Reward** | One reward per mission (MVP) | Free shipping, percentage discount, or fixed discount. |
| **Priority** | Integer, lower = higher | Used for conflict resolution when multiple missions/campaigns compete (Phase 26). |
| **Conditions** | MVP: campaign schedule + cart state | Category/product/role conditions expand in later phases. |

**Mission examples (roadmap reference):** cart amount, item quantity, distinct item count, category amount, category quantity, product quantity, weight, conditional combinations. **MVP ships:** amount, quantity, category. The engine (Phase 4) is designed so `product`, `weight`, `distinct quantity`, and `composite` missions can be added without re-architecting.

### 2.2 Reward

The **benefit unlocked by completing a mission**. Rewards are decoupled from mission calculation (Phase 5) so they can be changed, stacked, and safety-checked independently.

| Reward type | MVP status | Semantics |
|---|---|---|
| **Free shipping** | ✅ MVP | Waives shipping when the mission is met; must respect existing WooCommerce shipping zones/methods (compatibility with `free_shipping`/method settings). |
...
**Reward safety (non-negotiable, Phase 5):** prevent duplicate rewards, reward loops, rewards persisting after a mission becomes incomplete, invalid coupon application, unintended stacking, and application to excluded products.

### 2.3 Campaign

A **collection of missions active under specific conditions and schedules**. Campaigns let a store run a cohesive promotion (e.g. a summer sale with tiered milestones) instead of one-off missions.

- A campaign owns: `name`, `status`, `start date`, `end date`, `priority`, ordered missions (milestones), and display rules.
- Missions inside a campaign act as **milestones**: reaching each threshold unlocks that tier's reward.
- **Campaign scheduling** is in MVP scope: `start_date` / `end_date` activate and deactivate the campaign automatically.
- Conflict rules (overlapping campaigns, conflicting missions, best-reward vs cumulative) are defined deterministically in Phase 26; the admin UI must communicate the behavior.
...
The **current state relative to a mission**, always computed honestly from the live cart. Progress is a first-class data object, not a hard-coded percentage:

| Field | Meaning |
|---|---|
| `current` | Current cart value for the mission's basis (amount / quantity / category). |
| `target` | The mission threshold. |
| `remaining` | `target − current` (never negative). |
| `percentage` | `current / target` (clamped 0–100). |
| `completed` | Boolean — mission reached. |
| `reward_state` | Which reward applies and whether it is activated. |
| `eligible` | Whether the mission applies to this cart/shopper at all (+ reason when not). |

**Progress guarantees:** updates live via AJAX on cart changes; a mission that becomes incomplete *reverts* its reward state; no stale values are shown.

### 2.5 Suggestion

A **product recommendation designed to help the customer reach a mission** — the "close the gap" mechanic that turns motivation into revenue.

- **Sources (MVP):** products in the mission's category, related/cross-sell/upsell products, best sellers, manually selected products. (Recently viewed where available, later.)
- **Ranking (MVP):** stock availability → mission eligibility → price proximity to `remaining` → relevance → manual priority.
...
1. Create a mission (name, type, target, reward, message) in the React admin.
2. Bundle missions into a scheduled campaign (tiers/milestones).
...
|---|---|---|---|
| 1 | **Amount mission** | Threshold on the cart amount (configurable calculation basis; default subtotal). | 4 |
| 2 | **Quantity mission** | Threshold on total item quantity in cart. | 4 |
| 3 | **Category mission** | Amount or quantity restricted to one or more product categories. | 4 |
| 4 | **Multiple missions** | Several independent missions can be active simultaneously (each with its own progress/reward). | 4, 26 |
| 5 | **Milestone missions** | Ordered thresholds within a campaign, each unlocking its own reward tier. | 10 |

### 4.2 Rewards

| # | Feature | Requirement | Phase |
|---|---|---|---|
| 6 | **Free shipping reward** | Unlock free shipping on mission completion; honor WooCommerce shipping zones/methods. | 5 |
...
- **Post-MVP backlog** (from roadmap, prioritized): free gift → customer-selected gifts → advanced conditions → shipping-zone rules → customer segmentation → countdown → sticky bar → campaign templates → full analytics dashboard → A/B testing → revenue attribution → smart mission recommendation → margin-aware recommendations → AI campaign optimization.
- **V2 advanced features** (Phase 32): free gift selection, countdown, campaign templates, advanced conditional rules, customer roles, first-order/VIP missions, shipping-zone missions, brand/tag/attribute conditions, scheduled campaigns (advanced), celebration animations, advanced sticky bar, advanced upsell ranking.
...
- Every evaluator returns a consistent `MissionResult` (§2.4) regardless of mission type.
...
### 5.2 Rewards
- Decoupled from calculation; applied only while the mission is complete; reverted when it becomes incomplete.
- Stacking, deduplication, and exclusion rules enforced (Phase 5 “Reward Safety”).

### 5.3 Frontend
- Reusable component set: `MissionContainer` (one card per eligible mission),
  `ProgressBar`, `MissionMessage`, `RewardStatus`, `UnifiedRecommendations`
  (Suggestions + Upsells consolidation, Phase 14 + Phase 33.7),
  `StickyMissionBar` (Phase 11) — names following the reference project
  conventions. Every eligible mission renders as its own stacked card; the
  cross-mission `MissionMilestones` ladder was removed when that landed.
...
- Templates: the pluggable engine ships the six design templates `template-1` … `template-6` (classic progress card, minimal inline cart mission, circular progress, product recommendation + mission, compact floating/sticky mission, premium/elegant e-commerce style) — each with its own structure and schema-driven settings (colors, typography, border, radius, spacing, height, icons, animation, CSS class, custom CSS); the retired Phase 12 ids (`basic` / `percentage` / `milestone` / `card` / `ring`) are never mapped to a current template (a stored old id falls back to the default), and third parties register more via `faracart_template_classes` (see `docs/api.md` §3).
- Message variables: `{current} {target} {remaining} {percentage} {quantity} {remaining_quantity} {reward} {mission_name} {campaign_name}` (Phase 13).

### 5.4 Admin
- React admin shell (Phase 8) with pages: Dashboard, Missions, Campaigns, Analytics, Settings, Appearance.
- Mission CRUD + builder (Phase 9): basic info, type, target, reward, conditions, display, priority.
- Campaign builder (Phase 10): CRUD, mission/milestone ordering, activation, scheduling, priority, customer conditions, preview, duplicate.
...
- Settings (Phase 18): general, frontend, mission-calculation, performance, advanced (debug/logging/custom CSS/hooks).
...
- Basic analytics dashboard (Phase 17): impressions, completions, completion rate, revenue influenced, AOV, top campaigns/missions/suggested products, with date-range, campaign, mission, and reward filters; charts follow the reference plugin's charting conventions.
...
- **Mission completion rate** — completed missions ÷ impressions.
...
- **Adoption** — store owner can create a mission + campaign and see analytics without support.
...
- [x] Core product concepts — Mission, Reward, Campaign, Progress, Suggestion — defined with examples (P01-T02).
- [x] Initial MVP scope captured, including all 20 roadmap items and explicit deferrals of AI/A-B testing (P01-T03).
- [x] Spec written to `docs/PRODUCT_SPEC.md` and consistent with the roadmap (missions/rewards/campaigns/progress/suggestions/analytics).
