...
| `free_gift` | `gift_product_id`, `gift_add_mode` (`automatic` \| `optional`) | Automatic: gift line added with `goalcart_gift` cart data, price zeroed during totals, removed when the goal becomes incomplete. Optional: exposed through `RewardResult` meta for a single-click frontend action. |
