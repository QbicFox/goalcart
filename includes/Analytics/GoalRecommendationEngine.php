 ...
 *
 * Phase 33.4 (Smart Goal Recommendation) — answers "what threshold should
 ...
 * shipping for free-shipping goals). Each candidate is scored on four
 ...
 *  - history      15% — the store's existing goal completion performance
 *                       (neutral 50 without goal history)
 *
 ...
 * neutral with reduced confidence. The engine never changes a goal — the
 * admin must explicitly approve an application (P33-53).
 *
 * Extensibility (P33-60): the public recommend() contract is the frontend
 * contract — a future MLGoalRecommendationEngine can replace this class
...
final class GoalRecommendationEngine {
	 ...
	 * share of the threshold to complete the goal).
	 *
	 * @var float
	 */
	...
	/**
	 * Attribution engine (order values, shipping stats, goal funnel).
	 *
	...
	/**
	 * Goal repository (goal lookup for reward type + history).
	 *
	 * @var GoalRepository
	 */
	 ...
	 */
	public function __construct( AttributionEngine $engine, RewardCostEstimator $costs, GoalRepository $repository ) {
		$this->engine     = $engine;
		$this->costs      = $costs;
	...
	/**
	 * Whether smart goal recommendations are enabled.
	 *
	 * @return bool
	 */
	public function enabled() {
		/**
		 * Filter whether smart goal recommendations are on.
		 *
	...
	/**
	 * Recommend a goal threshold from the store's order data.
	 *
	...
	public function recommend( array $args = array() ) {
		...
		// synthetic goal built from the request args.
		$reward_type = $this->reward_type( $args, $goal );
		$reward_goal = null !== $goal ? $goal : $this->synthetic_goal( $args, $reward_type );
	 ...
	 * @param Goal|null            $reward_goal Goal carrying the reward config.
	 * @param array<string, mixed> $shipping    Shipping stats.
	 * @param array<string, mixed> $margin      Margin analysis.
	 * @param array<string, mixed>|null $history Goal funnel history.
	 * @return array<string, mixed> The scored candidate.
	 */
	protected function score_candidate( $threshold, array $stats, array $totals, $reward_type, $reward_goal, array $shipping, array $margin, $history ) {
		$aov    = (float) $stats['aov'];
		...
		if ( null !== $reward_goal && null !== $reward_type ) {
			...
			if ( $this->reward_needs_value( $reward_type ) && null === $reward_goal->reward_value() ) {
	...
	/**
	 * History score: the store's existing goal completion performance shows
	 * how attainable goals of this stretch are.
	 *
	 * @param array<string, mixed>|null $history Goal funnel history.
	 * @return float 0–100.
	 */
	 ...
	 * data and the depth of goal history — clamped so it never presents
 ...
 *
	 ...
	 * free-shipping goals (P33-20): AOV + average shipping and median +
	...
	/**
	 * Resolve the goal for a request (reward config + history source).
	 *
	...
	/**
	 * The reward type to recommend for: explicit arg > the goal's own.
	 *
	 * @param array<string, mixed> $args Request args.
	 * @param Goal|null            $goal Resolved goal.
	 * @return string|null
	 */
	protected function reward_type( array $args, $goal ) {
		if ( ! empty( $args['reward_type'] ) ) {
			return (string) $args['reward_type'];
		}

		return null !== $goal ? $goal->reward_type() : null;
	}

	/**
	 * A synthetic goal carrying the request's reward config, for
	 * recommendations made without an existing goal.
	 *
	 * @param array<string, mixed> $args        Request args.
	 * @param string|null          $reward_type Reward type.
	 * @return Goal|null
	 */
	protected function synthetic_goal( array $args, $reward_type ) {
		if ( null === $reward_type ) {
			return null;
		}
