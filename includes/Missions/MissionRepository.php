<?php
/**
 * Mission repository for the FaraCart engine.
 *
 * @package FaraCart
 */
...
final class MissionRepository {
	...
	/**
	 * The campaign-priority a standalone mission competes at.
	 *
	...
	/**
	 * Per-request cache of active missions.
	 *
	 * @var Mission[]|null
	 */
	protected $cache;
	...
	/**
	 * All missions currently eligible to run (status active, campaign active).
	 *
	...
	public function active_missions() {
		...
		// mission so a mission inside an inactive or out-of-schedule campaign can
		...
		// one regardless of the individual missions' priorities.
		...
		foreach ( (array) $rows as $row ) {
			$mission = new Mission( $this->normalize( $row ) );
	 ...
	 * Find a single mission by id (engine path: campaign folding applied).
	 *
	 * Runs the campaign join (unlike get(), which returns stored values for
	 * the admin CRUD), so the returned Mission reflects the effective
	 * campaign gating exactly like active_missions().
	 *
	 * @param int $mission_id Mission id.
	 * @return Mission|null Null when the mission does not exist.
	 */
	public function find( $mission_id ) {
		global $wpdb;
		...
		return new Mission( $this->normalize( $row ) );
	}
...
public function all( array $args = array() ) {
	...
	// Admin list shows the STORED mission values (stored status/dates), so
	// the CRUD UI never sees campaign-folded state; the engine applies the
	// campaign gating itself via active_missions()/find().
	foreach ( (array) $rows as $row ) {
	...
	/**
	 * Mission ids belonging to a campaign.
	 *
	 * Used by the analytics purchase metrics to resolve the campaign filter
	 * onto the attribution layer (missions are the attribution dimension).
	 *
	...
	/**
	 * Mission ids carrying a reward type.
	 *
	...
	/**
	 * Query mission ids with a prepared WHERE clause.
	 *
	...
	/**
	 * Get a single mission as a normalized array (REST detail payload).
	 *
	 * @param int $mission_id Mission id.
	 * @return array<string, mixed>|null Null when the mission does not exist.
	 */
	public function get( $mission_id ) {
	global $wpdb;

	$missions = Schema::table( 'missions' );
	$row   = $wpdb->get_row(
		$wpdb->prepare( "SELECT * FROM {$missions} WHERE id = %d", (int) $mission_id ),
		ARRAY_A
	);
...
/**
 * Create a mission from a validated payload.
 *
 * Maps the REST payload keys onto the missions table columns (conditions
 * keys packed into the conditions JSON). Returns the new mission id, or 0
 * when the insert failed.
 *
 * @param array<string, mixed> $data Mission payload (superset of columns).
 * @return int New mission id, 0 on failure.
 */
public function create( array $data ) {
	...
	$inserted = $wpdb->insert( Schema::table( 'missions' ), $row );

	if ( $inserted ) {
		/**
		 * Fires after a mission is created.
		 *
...
/**
 * Update a mission from a validated partial payload.
 *
 * Only the keys present in $data are written. Returns true when the
 * update succeeded or had nothing to change (idempotent).
 *
 * @param int                    $mission_id Mission id.
 * @param array<string, mixed>   $data    Partial mission payload.
 * @return bool
 */
public function update( $mission_id, array $data ) {
	global $wpdb;
	...
	if ( false !== $updated ) {
		/**
		 * Fires after a mission is updated.
		 *
		 * @param int $mission_id Mission id.
		 */
 ...
 * Delete a mission.
 *
 * Hard delete — analytics history survives because the event log's mission
...
public function delete( $mission_id ) {
	...
	if ( false !== $deleted ) {
		/**
		 * Fires after a mission is deleted.
		 *
		 * @param int $mission_id Mission id.
		 */
		do_action( 'faracart_missions_changed', (int) $mission_id );
	 ...
	 * Duplicate a mission (copy with a ' (copy)' name suffix).
	 *
	 * Copies the STORED values (stored status/dates) — the copy never
	 * inherits the source's campaign-folded engine state.
	 *
	 * @param int $mission_id Mission id.
	 * @return int New mission id, 0 when the source mission does not exist or the
	 *             insert failed.
	 */
	public function duplicate( $mission_id ) {
		$row = $this->get( (int) $mission_id );
	...
	$row['name'] = sprintf(
		/* translators: %s: original mission name. */
		__( '%s (copy)', 'faracart' ),
		$row['name']
	);
...
/**
 * Map payload keys onto missions table columns.
 *
 ...
 *
 * @param array<string, mixed> $data Mission payload.
...
protected function sanitize_column( $type, $value ) {
	...
	/**
	 * Fold the campaign status/schedule onto a decoded mission row.
	 *
	 * Engine path: a mission's own dates take precedence; otherwise the
	 ...
	 *
	 * @param array<string, mixed> $row Decoded mission row.
	 * @return array<string, mixed>
	 */
	protected function fold_campaign( array $row ) {
		if ( ! empty( $row['campaign_id'] ) ) {
			if ( ! empty( $row['campaign_status'] ) && Mission::STATUS_ACTIVE !== $row['campaign_status'] ) {
				$row['status'] = Mission::STATUS_INACTIVE;
			}
	...
	/**
	 * Normalize a raw database row into a Mission constructor payload.
	 *
