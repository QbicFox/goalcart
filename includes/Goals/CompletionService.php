 ...
 *
 * The single authoritative implementation of the per-user goal completion
 * limit ("how many times may each shopper complete this goal?").
 *
 * Every goal may configure `max_completions_per_user` (NULL = unlimited).
 * The rule lives HERE — never duplicated in the frontend, AJAX handlers,
 * REST endpoints, reward handlers or checkout hooks:
 *
 *  - `can_complete()`      — the business rule: goal active? unlimited?
 *                            count < limit?
 *  - `count_for()` / `counts_for()` — efficient COUNT queries over the
 *                            `goal_completions` history (indexed by
 *                            goal + identity), never a full-table load.
 *  - `record_completion()` — the write path: count + insert inside a
 *                            transaction with a row lock on the goal, so
 *                            concurrent requests cannot exceed the limit
 *                            (the `order_goal` unique key additionally
 ...
 * enforcement limit. `goal_completions` is a dedicated, server-written
 * log: one row per (goal, order, identity) recorded when an order
 ...
 * normal goal lifecycle; a completion is ONE successful cycle (an order
 * that met the goal). Progress and the completion count stay separate —
...
final class CompletionService {
	...
	/**
	 * Goal repository (active goals for order-time recording).
	 *
	...
	/**
	 * Goal engine (order-time evaluation).
	 *
	 * @var GoalEngine|null
	 */
	protected $engine;
	...
	/**
	 * Whether the goal_completions table exists (checked once per request).
	 *
	 * @var bool|null
	 */
	...
	/**
	 * Per-request completion-count cache: goal_id => count for the
	 ...
	 */
	public function __construct( ?Settings $settings = null, ?GoalRepository $repository = null, ?GoalEngine $engine = null, ?Session $session = null ) {
	 ...
	 *    (the `order_goal` unique key makes re-processing a no-op).
	 *
	 ...
	 *
	 * Gated on the same master `enabled` setting every other goal feature
	 * respects — a disabled plugin records no completions and grants no
	 * goal rewards, so the limit stays consistent with the storefront.
	 *
	 * @return bool
	 */
	...
	/**
	 * How many times this identity has completed the goal.
	 *
	 ...
	 * @return int
	 */
	public function count_for( $goal_id, $user_id, $session_id, $fresh = false ) {
		$goal_id    = (int) $goal_id;
		$user_id    = (int) $user_id;
		...
		if ( ! $fresh && $this->count_cache_identity === $identity_key && isset( $this->count_cache[ $goal_id ] ) ) {
			return $this->count_cache[ $goal_id ];
		}
		...
		$table = Schema::table( 'goal_completions' );

		if ( $user_id > 0 ) {
			$count = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$table} WHERE goal_id = %d AND user_id = %d",
					$goal_id,
					$user_id
				)
			);
			...
			$count = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$table} WHERE goal_id = %d AND session_id = %s",
					$goal_id,
					$session_id
				)
			);
		...
		if ( ! $fresh ) {
			...
			$this->count_cache[ $goal_id ] = $count;
		}
	...
	/**
	 * Batch completion counts for several goals and one identity.
	 *
	 * One grouped COUNT query instead of one per goal — the storefront
	 * payload path (Phase 22: only calculate when goal/user context
	 * requires it, and then cheaply).
	 *
	 * @param int[]  $goal_ids    Goal ids.
	 * @param int    $user_id     Logged-in user id (0 = guest).
	 * @param string $session_id  Anonymous session id (guests).
	 * @return array<int, int> goal_id => completion count.
	 */
	public function counts_for( array $goal_ids, $user_id, $session_id ) {
		$user_id    = (int) $user_id;
		$session_id = Session::is_valid( $session_id ) ? $session_id : '';
		$identity_key = ( $user_id > 0 ? 'u' . $user_id : 's' . $session_id );

		$ids = array_values( array_filter( array_map( 'intval', $goal_ids ), function ( $id ) {
			return $id > 0;
		} ) );
		...
		if ( $this->count_cache_identity === $identity_key ) {
			...
			foreach ( $ids as $goal_id ) {
				if ( isset( $this->count_cache[ $goal_id ] ) ) {
					$result[ $goal_id ] = $this->count_cache[ $goal_id ];
				}
			}
		...
		if ( $user_id > 0 ) {
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT goal_id, COUNT(*) AS cnt FROM {$table} WHERE goal_id IN ({$in}) AND user_id = %d GROUP BY goal_id",
			...
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT goal_id, COUNT(*) AS cnt FROM {$table} WHERE goal_id IN ({$in}) AND session_id = %s GROUP BY goal_id",
		...
		foreach ( (array) $rows as $row ) {
			$counts[ (int) $row['goal_id'] ] = (int) $row['cnt'];
		}

		// Prime the per-request cache so later single-goal reads reuse this.
		$this->count_cache_identity = $identity_key;

		foreach ( $ids as $goal_id ) {
			$this->count_cache[ $goal_id ] = isset( $counts[ $goal_id ] ) ? $counts[ $goal_id ] : 0;
		}

		foreach ( $counts as $goal_id => $count ) {
			$this->count_cache[ $goal_id ] = $count;
		}
	...
	/**
	 * The authoritative completion status for a goal + identity.
	 *
	 ...
	 */
	public function status( Goal $goal, $user_id, $session_id ) {
		$limit = $goal->max_completions_per_user();
		$count = $this->count_for( $goal->id(), $user_id, $session_id );
	...
	/**
	 * Completion status for a goal + cart context.
	 *
	 * @param Goal        $goal    Goal.
	 * @param CartContext $context Cart snapshot.
	 * @return array{completion_limit: int|null, completion_count: int, remaining_completions: int|null, can_complete: bool}
	 */
	public function context_status( Goal $goal, CartContext $context ) {
		$identity = $this->context_identity( $context );

		return $this->status( $goal, $identity['user_id'], $identity['session_id'] );
	}

	/**
	 * Batch completion statuses for several goals + one cart context.
	 *
	 * @param Goal[]      $goals   Goals.
	 * @param CartContext $context Cart snapshot.
	 * @return array<int, array{completion_limit: int|null, completion_count: int, remaining_completions: int|null, can_complete: bool}> goal_id => status.
	 */
	public function context_statuses( array $goals, CartContext $context ) {
		$identity = $this->context_identity( $context );
		...
		foreach ( $goals_by_id as $goal_id => $goal ) {
			$limit = $goal->max_completions_per_user();
			$count = isset( $counts[ $goal_id ] ) ? $counts[ $goal_id ] : 0;
	 ...
	 * goal?
	 *
	 *   goal inactive?           -> cannot complete
	 *   goal unlimited?          -> allowed (no count query — the default
	 *                                for every existing goal, so the limit
	 ...
	 * @return bool
	 */
	public function can_complete( Goal $goal, $user_id, $session_id ) {
		if ( ! $goal->is_active() ) {
			return false;
		}
		...
		return $this->count_for( $goal->id(), $user_id, $session_id ) < $limit;
	}

	/**
	 * Whether a goal is still completable for the cart context.
	 *
	 * @param Goal        $goal    Goal.
	 * @param CartContext $context Cart snapshot.
	 * @return bool
	 */
	public function context_allows( Goal $goal, CartContext $context ) {
		$identity = $this->context_identity( $context );

		return $this->can_complete( $goal, $identity['user_id'], $identity['session_id'] );
	}

	/**
	 * Filter goals down to those the context's identity may still complete.
	 *
	 * Used by the reward engine so an exhausted goal can never grant its
	 * reward (and any previously applied reward is revoked by the normal
	 * reconcile pass). Unlimited goals pass through without a query.
	 *
	 * @param Goal[]      $goals   Goals.
	 * @param CartContext $context Cart snapshot.
	 * @return Goal[]
	 */
	public function available_goals( array $goals, CartContext $context ) {
		$identity = $this->context_identity( $context );

		$limited = array_filter( $goals, function ( Goal $goal ) {
			return null !== $goal->max_completions_per_user();
		} );

		// No goal carries a limit: everything stays, zero queries.
		if ( empty( $limited ) ) {
			return array_values( $goals );
		}

		$counts = $this->counts_for(
			array_map( function ( Goal $goal ) {
				return $goal->id();
			}, $limited ),
			$identity['user_id'],
			$identity['session_id']
		...
		return array_values(
			array_filter( $goals, function ( Goal $goal ) use ( $counts ) {
				$limit = $goal->max_completions_per_user();
				...
				$count = isset( $counts[ $goal->id() ] ) ? $counts[ $goal->id() ] : 0;
	 ...
	 *
	 * For a limited goal the count + insert run inside a transaction with
	 * a row lock on the goal (`SELECT ... FOR UPDATE`), so two concurrent
	 * requests both see the pre-insert count and exactly one of them can
	 * cross the limit — the invariant
	 * `successful completions for one identity + one goal <= limit`
	 * always holds. The `order_goal` unique key additionally makes the
	 * same (order, goal) exactly-once regardless of which hook path or
	 ...
	 * @param string|null $reward_type The goal's reward type at completion.
	 ...
	 */
	public function record_completion( Goal $goal, $user_id, $session_id, $order_id = 0, $reward_type = null ) {
		$user_id    = (int) $user_id;
		$order_id   = (int) $order_id;
		$session_id = Session::is_valid( $session_id ) ? $session_id : '';
		$identity_key = ( $user_id > 0 ? 'u' . $user_id : 's' . $session_id );

		if ( $goal->id() < 1 || '' === $identity_key || ! $this->table_ready() ) {
			return false;
		}

		$limit = $goal->max_completions_per_user();

		// Unlimited goals have no cap to enforce; the order_goal unique
		// key still dedups double-processing of the same order.
		if ( null === $limit ) {
			$inserted = $this->insert_completion( $goal, $user_id, $session_id, $order_id, $reward_type );

			if ( $inserted ) {
				$this->refresh_count_cache( $goal->id(), $user_id, $session_id, $identity_key );
			}
		...
		try {
			// Lock the goal row so concurrent attempts serialize on it.
			$locked = $wpdb->get_var(
				$wpdb->prepare( "SELECT id FROM {$goals_table} WHERE id = %d FOR UPDATE", $goal->id() )
			);
			...
			if ( $this->count_for( $goal->id(), $user_id, $session_id, true ) >= $limit ) {
				$wpdb->query( 'ROLLBACK' ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	/**
	 * Refresh the per-request count cache for one goal after a successful
	 ...
	 * @return void
	 */
	protected function refresh_count_cache( $goal_id, $user_id, $session_id, $identity_key ) {
		$this->count_cache_identity = $identity_key;
		$this->count_cache[ (int) $goal_id ] = $this->count_for( $goal_id, $user_id, $session_id, true );
	}
	 ...
	 * @return bool
	 */
	protected function insert_completion( Goal $goal, $user_id, $session_id, $order_id, $reward_type ) {
		global $wpdb;
		...
		// A unique-key violation (same order + goal already recorded) is an
	 ...
	 * For every active goal the order met, records a completion cycle for
	 * the order's identity — honoring the per-user limit (an exhausted
	 * goal records nothing and grants nothing).
	 *
	 * @param int $order_id Order id.
	...
	public function record_order_completions( $order_id, $order = null ) {
		...
		foreach ( $goals as $goal ) {
			...
			if ( $this->record_completion( $goal, $user_id, $session_id, $order_id, $goal->reward_type() ) ) {
				$recorded++;
			}
		}
	 ...
	 *
	 * The goal engine evaluates against the same normalized snapshot it
